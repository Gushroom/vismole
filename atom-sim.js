...
function Bond(bond) {
    this.type = bond[0];
    this.atom1Label = bond[1];
    this.atom2Label = bond[2];
}

const atoms = new Array();
const bonds = new Array();
function extractData() {


    for (let item of jsonInput.items) {
        atoms.push(new Atom(item));
    }


    for (let bond of jsonInput.bonds) {
        bonds.push(new Bond(bond));
    }
}
...
// document.head.append('<script type= "text/javascript" src="three.js"></script>');

let renderer, camera, controls, axes, grids;
// Define lights
...
function drawAtoms() {
    const ATOMCOLOR = 0xffff00;
    
    for (let atom of atoms) {
        let atomGeometry = new THREE.SphereGeometry(0.5, 16, 16);
        let atomMaterial = new THREE.MeshLambertMaterial({ color: ATOMCOLOR });
        atomGeometry.translate(atom.x, atom.y, atom.z);
        let atomMesh = new THREE.Mesh(atomGeometry, atomMaterial);
        scene.add(atomMesh);
    }
}
...
function drawBonds() {
    for (let bond of bonds){
        al1 = bond.atom1Label
        al2 = bond.atom2Label
        atom1 = atoms.find(item=>item.label==al1)
        atom2 = atoms.find(item=>item.label==al2)

        let bondGeometry = new THREE.CylinderGeometry(0.1, 0.1, )
    }
}
